...
async function downloadBackup() {
  ...
  ipcRenderer.send("downloadBackup", {
  ...
  <div class="dapp-list mt-2" style="text-align: center; margin-top: auto; margin-bottom: auto">
    ...
    <ui-grid class="row px-4">
      ...
      <ui-grid-cell columns="6">
        <ui-button class="step_btn" type="button" raised @click="downloadBackup">
          {{ t("common.backup_btn") }} </ui-button
        ><br />
        <router-link :to="'/dashboard'" style="text-decoration: none" replace>
          <ui-button outlined class="step_btn"> Exit settings menu </ui-button>
        </router-link>
      </ui-grid-cell>
      <ui-grid-cell columns="3" />
    </ui-grid>
