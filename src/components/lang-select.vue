...
let selected = ref(store.state.SettingsStore.settings.locale?.iso ?? defaultLocale.iso);
...
<template>
  <ui-menu-anchor v-if="location === 'prompt'" absolute>
    <ui-button raised icon="translate" @click="menuClick">
    ...
    <ui-menu
      ...
      @selected="onSelected"
    >
      <ui-menuitem v-for="item in selectLocales" :key="item.value" :value="item.value">
        {{ item.label }}
      </ui-menuitem>
    </ui-menu>
  </ui-menu-anchor>
  <ui-menu-anchor v-else absolute position="bottom start">
    <ui-fab icon="translate" mini @click="menuClick" />
    <ui-menu
      v-model="open"
      ...
      @selected="onSelected"
    >
      <ui-menuitem v-for="locale in selectLocales" :key="locale.value" :value="locale.label">
        {{ locale.label }}
      </ui-menuitem>
    </ui-menu>
  </ui-menu-anchor>
