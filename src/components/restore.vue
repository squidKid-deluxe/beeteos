  ...
  fs.readFile(file, "utf-8", async (err, data) => {
    ...
    try {
      decryptedData = await aes.decrypt(
        data,
        legacy.value ? backupPass.value : sha512(backupPass.value).toString()
      );
    } catch (error) {
      console.log(error);
      fileError.value = true;
      ...
      store.dispatch("WalletStore/notifyUser", {
        notify: "request",
        message: t("common.apiUtils.restore.decryptError"),
      });
      return;
    }
    ...
    try {
      ...
      store.dispatch("WalletStore/notifyUser", {
        notify: "request",
        message: t("common.apiUtils.restore.invalidPassword"),
      });
      return;
    }
    ...
    if (existingWalletNames.includes(parsedData.wallet)) {
      ...
      store.dispatch("WalletStore/notifyUser", {
        notify: "request",
        message: t("common.apiUtils.restore.duplicate"),
      });
      return;
    }

    try {
      await store.dispatch("WalletStore/restoreWallet", {
        backup: parsedData,
...
<template>
  <div class="bottom p-0">
    <div class="content px-3">
      ...
      <p v-tooltip="t('common.tooltip_backupfile_cta')" class="my-3 font-weight-bold">
      ...
      <input
        ...
        required
      />
      <p v-tooltip="t('common.tooltip_backuppass_cta')" class="my-3 font-weight-bold">
