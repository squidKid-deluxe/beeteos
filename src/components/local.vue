...
<template>
  <div v-if="settingsRows" class="bottom p-0">
    <span v-if="supportsLocal">
      <span>
        <AccountSelect />
        <p v-if="!opPermissions" style="marginbottom: 0px">
          {{ t("common.local.label") }}
        </p>
        <ui-card v-if="!selectedRows" v-shadow="3" outlined style="margintop: 5px">
      ...
      <span v-if="opPermissions && settingsRows && selectedRows">
        <span v-if="!inProgress">
          ...
          <ui-file accept="application/json" @change="onChange($event)" />
        </span>
        <span v-else>
          <ui-spinner active />
