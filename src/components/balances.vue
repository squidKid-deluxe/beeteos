  ...
  if (balances.value && balances.value.length) {
    tableData.value = {
      data: balances.value.map((balance) => {
        return {
          balance: balance.balance.toLocaleString(undefined, {
...
<template>
  <div style="padding: 5px">
    ...
    <ui-button v-if="isConnected || balances" class="step_btn" @click="loadBalances()">
      {{ t("common.balances.refresh") }}
    </ui-button>
    <ui-button v-else-if="!isConnected && !isConnecting" class="step_btn" @click="loadBalances()">
    ...
    <ui-table
      ...
      style="height: 180px"
    />
    <ui-card v-if="balances && !balances.length" v-shadow="1" outlined>
      {{ t("common.balances.empty") }}
    </ui-card>
    <ui-card v-if="isConnecting" v-shadow="1" outlined style="padding: 5px; text-align: center">
      <ui-skeleton active />
    </ui-card>
    <ui-card v-if="!isConnected && !isConnecting" v-shadow="1" outlined style="padding: 5px">
