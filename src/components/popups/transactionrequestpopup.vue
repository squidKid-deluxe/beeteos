...
let tableTooltip = computed(() => {
  ...
  return t("operations.rawsig.request", {
    appName: props.request.payload.appName,
    origin: props.request.payload.origin,
    chain: formatChain(props.request.payload.chain),
    accountName: props.visualizedAccount
      ? props.visualizedAccount
      : props.request.payload.account_id,
  });
});
...
<template>
  ...
  <div v-if="!!visualizedParams" class="text-left custom-content" style="margintop: 10px">
    <ui-card>
      <ui-card-content>
        <ui-card-text>
          ...
          >
            {{
              t(
                `operations.injected.BTS.${visualizedParams[page - 1].method}.rows.${row.key}`,
                row.params
              )
            }}
          </div>
      ...
      <ui-card-actions>
        <ui-card-buttons>
          <ui-button outlined @click="open = true">
    ...
    <ui-switch v-model="receipt" input-id="enable-receipt" style="margin-bottom: 5px" />
    <label :for="'enable-receipt'" style="margin-left: 15px">
    ...
    <div v-if="!!visualizedParams" style="padding-bottom: 25px">
      <ui-button raised style="margin-right: 5px" @click="_clickedAllow()">
        {{ buttonText }}
      </ui-button>
    ...
    <div v-else style="padding-bottom: 25px">
      <ui-button raised style="margin-right: 5px" disabled>
        {{ buttonText }}
      </ui-button>
      <ui-button raised @click="_clickedDeny()">
        {{ t("operations.rawsig.reject_btn") }}
  ...
  <div v-else class="text-left custom-content">
    <pre>
