...
import Operations from "./blockchains/operations.vue";
...
watchEffect(() => {
  if (timestamp && timestamp.value) {
    let msg = uuidv4();
    let shaMSG = sha512(msg + timestamp.value.getTime())
      .toString()
      .substring(0, 15);
...
ipcRenderer.on("deeplink", async (event, args) => {
  ...
  let blockchainActions = [Actions.TRANSFER, Actions.VOTE_FOR, Actions.INJECTED_CALL];
...
<template>
  <div v-if="settingsRows" class="bottom p-0">
    <span v-if="supportsTOTP">
      ...
      <span v-if="deepLinkInProgress">
        ...
        <ui-card v-shadow="3" outlined style="margintop: 5px">
          <br />
        ...
        <ui-card v-shadow="3" outlined style="margintop: 5px">
          ...
          <span v-if="opPermissions && settingsRows && selectedRows">
            ...
            >
              ...
              <ui-button raised style="margin-bottom: 10px" @click="setTime(600)"> 10m </ui-button>
            </span>
            <span>
              <ui-button
                v-if="!newCodeRequested && settingsRows && settingsRows.length > 0 && timeLimit"
