...
function unlockWallet() {
  store
    ...
    .catch(() => {
      ...
      ipcRenderer.send("notify", t("common.start.invalid_password"));
    });
}
</script>
...
<template>
  <div class="bottom">
    <div class="content">
      <p v-if="!hasWallet" class="mt-3 mb-3 font-weight-normal">
        <em>{{ t("common.no_wallet") }}</em>
      </p>

      <router-link v-if="!hasWallet" to="/create" replace>
        <ui-button raised>
          {{ t("common.start_cta") }}
        </ui-button>
      </router-link>

      <p v-if="!hasWallet" class="my-2 font-weight-normal">
        <em>{{ t("common.restore_lbl") }}</em>
      </p>

      <router-link v-if="!hasWallet" to="/restore" replace>
        <ui-button raised>
      ...
      <br />
      <ui-button v-if="hasWallet" type="submit" raised @click="unlockWallet">
        {{ t("common.unlock_cta") }}
      </ui-button>
    ...
    <p class="mt-2 mb-2 small">&copy; 2019-2023 BitShares</p>
  </div>
</template>
