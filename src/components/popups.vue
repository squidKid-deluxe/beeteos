<script setup lang="js">
  ...
  import LinkRequestPopup from "./popups/linkrequestpopup.vue";
  import ReLinkRequestPopup from "./popups/relinkrequestpopup.vue";
  import IdentityRequestPopup from "./popups/identityrequestpopup.vue";
  import GenericRequestPopup from "./popups/genericrequestpopup.vue";
  import SignMessageRequestPopup from "./popups/signedmessagepopup.vue";
  import TransactionRequestPopup from "./popups/transactionrequestpopup.vue";
  import TransferRequestPopup from "./popups/transferrequestpopup.vue";
...
<template>
  <div v-if="type && type !== '' && request" style="overflow-y: auto; width: 750px">
    <ui-collapse with-icon ripple model-value="{{True}}">
      ...
      <SignMessageRequestPopup
        v-else-if="
          (type === Actions.SIGN_MESSAGE || type === Actions.SIGN_NFT) && request && accounts
        "
        :request="request"
        :accounts="accounts"
      />
      <TransferRequestPopup
        v-else-if="type === Actions.TRANSFER && request && chain && accountName && target && toSend"
        :request="request"
        :chain="chain"
    ...
    <ui-collapse v-if="type === Actions.REQUEST_LINK && chainOperations" with-icon ripple>
      <template #toggle>
    ...
    <ui-collapse v-if="moreRequest" with-icon ripple>
      ...
      <div>
        <ui-textfield v-model="moreRequest" input-type="textarea" fullwidth disabled rows="8" />
      </div>
    </ui-collapse>
    <ui-collapse v-if="payload" with-icon ripple>
      ...
      <div>
        <ui-textfield v-model="payload" input-type="textarea" fullwidth disabled rows="8" />
      </div>
    </ui-collapse>
  ...
  <div v-else>Error: Unable to load prompt.</div>
</template>
