<script setup lang="js">
  ...
  watchEffect(() => {
    ...
    if (visualizedParams.value && visualizedParams.value.length) {
      jsonData.value = JSON.stringify(visualizedParams.value[currentPageValue].op, undefined, 4);
    }

    if (result.value && result.value.length) {
      resultData.value = JSON.stringify(
        result.value[0].trx.operation_results[currentPageValue],
        undefined,
        4
      );
...
<template>
  <div style="overflow-y: auto; width: 750px">
    <ui-collapse with-icon ripple model-value="{{True}}">
      ...
      <div style="overflow-y: auto; padding-right: 25px; padding-left: 5px; padding-bottom: 15px">
        ...
        <div v-if="!!visualizedParams" class="text-left custom-content" style="margintop: 10px">
          <ui-card no-hover>
            <ui-card-content>
              <ui-card-text>
                <div v-if="visualizedParams.length > 1" :class="$tt('subtitle1')">
                  <b>{{ t(visualizedParams[page > 0 ? page - 1 : 0].title) }}</b> ({{ page }}/{{
                ...
                <div style="margin-bottom: 5px">
                  {{
                    t(
                      `operations.injected.BTS.${
                        visualizedParams[page > 0 ? page - 1 : 0].method
                      }.headers.result`
                    )
                  }}
                </div>
            ...
            <ui-card-actions>
              <ui-card-buttons>
                ...
                <ui-button outlined style="margin-left: 25px" @click="openOPRes = true">
                  {{ t("common.popup.result") }}
                </ui-button>
                <ui-button outlined style="margin-left: 25px" @click="openOpDetails = true">
  ...
  <ui-dialog v-model="openOPReq" fullscreen>
    ...
    <ui-dialog-content>
      <ui-textfield v-model="jsonData" input-type="textarea" fullwidth disabled rows="8" />
    </ui-dialog-content>
  </ui-dialog>
  ...
  <ui-dialog v-model="openOPRes" fullscreen>
    ...
    <ui-dialog-content>
      <ui-textfield v-model="resultData" input-type="textarea" fullwidth disabled rows="8" />
    </ui-dialog-content>
  </ui-dialog>
  ...
  <ui-dialog v-model="openResult" fullscreen>
    ...
    <ui-dialog-content>
      <ui-textfield v-model="moreResult" input-type="textarea" fullwidth disabled rows="8" />
    </ui-dialog-content>
  </ui-dialog>
