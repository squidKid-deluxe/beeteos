...
import ImportKeys from "./blockchains/ImportKeys.vue";
...
let selectedImportOption = computed(() => {
  ...
  let useImport =
    !selectedImport.value || !selectedImport.value
  ...
  return getBlockchainAPI(selectedChain.value)
    .getImportOptions()
    .find((option) => {
      return option.type == useImport.type;
    });
});
    ...
    <div class="content px-3">
      ...
      <div v-if="step == 1" id="step1">
        <template v-if="createNewWallet">
          <p v-tooltip="t('common.tooltip_friendly_cta')" class="my-3 font-weight-bold">
        ...
        <p v-tooltip="t('common.tooltip_chain_cta')" class="my-3 font-weight-bold">
          {{ t("common.chain_cta") }} &#10068;
        </p>
        <select
          id="chain-select"
        ...
        >
          ...
          <option v-for="chain in chainList" :key="chain.identifier" :value="chain.identifier">
            <span v-if="chain.testnet"> Testnet: {{ chain.name }} ({{ chain.identifier }}) </span>
            <span v-else> {{ chain.name }} ({{ chain.identifier }}) </span>
          </option>
        </select>
        <div v-if="selectedImportOptions.length > 0">
          ...
          <select
            ...
            required
          >
            <option key="0" selected disabled value="0">
              {{ t("common.import_placeholder") }}
            </option>
            <option v-for="option in selectedImportOptions" :key="option.type" :value="option">
        ...
        <ui-grid>
          <ui-grid-cell columns="12">
            <router-link :to="createNewWallet ? '/' : '/dashboard'" replace>
              <ui-button raised class="step_btn">
                {{ t("common.cancel_btn") }}
              </ui-button>
            </router-link>
      ...
      <div v-else-if="step == 2" id="step2">
        <ImportKeys
          ...
          :chain="selectedChain"
        />
        <div v-else>No import option found</div>
      </div>
      <div v-else-if="step == 3" id="step3">
        <div>
          <p v-tooltip="t('common.tooltip_password_cta')" class="mb-2 font-weight-bold">
            <span v-if="createNewWallet"> {{ t("common.password_cta") }} &#10068; </span>
            <span v-else> {{ t("common.unlock_with_password_cta") }} &#10068; </span>
          </p>
          <input
            id="inputPass"
            v-model="password"
