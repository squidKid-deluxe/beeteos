...
<template>
  <div>
    <span v-if="!QRresult && camera !== 'off' && !cameraError" style="height: 300px">
      <p>
        ...
        >
          ...
          >
            <span v-if="cameraInitializing">
              <ui-spinner style="padding-top: 65px" active />
            </span>
            ...
            <video class="qrcode-stream-camera" autoplay playsinline object-fit="cover" />
          </qrcode-stream>
        </ui-card>
      </div>
    ...
    <span v-else>
      <span v-if="cameraError">
        <p>Your webcam failed to initialize, please try again.</p>
        <ui-button @click="switchCamera"> Try again </ui-button>
      </span>
      <span v-else>
        <p>QR code scanned!</p>
        <ui-button @click="switchCamera"> Scan another </ui-button>
      </span>
    </span>
  </div>
</template>
