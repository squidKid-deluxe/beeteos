...
import { app, BrowserWindow, Menu, Tray, dialog, ipcMain, Notification } from "electron";
...
import { initApplicationMenu } from "../src/lib/applicationMenu";
import { getSignature } from "../src/lib/SecureRemote";
  ...
  ipcMain.on("downloadBackup", async (event, arg) => {
    ...
    dialog
      ...
      .then(async (result) => {
        ...
        let isValid;
        try {
          isValid = await secp.verify(response.signedMessage, response.msgHash, response.pubk);
        } catch (error) {
          console.log(error);
          return;
        }
